...
bool PxPackageSerializer::Write(const PxPackageFileTable & inTable)
{
	...
	DWORD dataOffset = CalculateDataOffset(inTable);
	...
	for (DWORD i = 0; i < inTable.NumEntries; ++i)
	{
		...
		while (entry)
		{
			...
			if (bufferPtr)
			{
				mMappedFile->Write(bufferPtr, entry->Size);
			}
		...
		if (numFiles > 0)
		{
			for (DWORD j = 0; j < numFiles; ++j)
			{
				...
				if (outHeader.Files[i] == nullptr)
					...
					outHeader.Files[max(0, j - 1)]->Next = entry;
			}
		}
	}
...
DWORD PxPackageSerializer::CalculateDataOffset(const PxPackageFileTable &inTable) const
{
	DWORD offset = 0;
